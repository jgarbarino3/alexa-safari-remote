...
class AgentProcessingTest(unittest.TestCase):
    ...
    def test_love_island_macro_runs_phases_and_injects_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("resume-love-island-peacock skill", sqs_agent.prompt)
            self.assertIn("Surfshark USA Fastest was attempted", sqs_agent.prompt)
            self.assertIn("Peacock is already open in Chrome", sqs_agent.prompt)
            self.assertIn("select Lillia", sqs_agent.prompt)
            self.assertIn("leave Chrome frontmost in fullscreen", sqs_agent.prompt)
            self.assertLess(len(sqs_agent.prompt), 320)
            log_text = (root / "agent.log").read_text(encoding="utf-8")
    ...
    def test_love_island_prompt_text_sets_peacock_rules(self):
        ...
        self.assertIn("resume-love-island-peacock skill", prompt)
        self.assertIn("Surfshark USA Fastest was attempted", prompt)
        self.assertIn("Peacock is already open in Chrome", prompt)
        self.assertIn("select Lillia", prompt)
        self.assertIn("leave Chrome frontmost in fullscreen", prompt)
